...
client = MongoClient(f'mongodb+srv://{username}:{password}@precificacao.axzys.mongodb.net/?retryWrites=true&w=majority&appName=Precificacao',serverSelectionTimeoutMS=5000, socketTimeoutMS=5000)
...
def main():
    ...
    if st.session_state['login']:
        if st.session_state['tipo_usuario'] == 1:
            opcoes = ["Configurações","Avulsos", "Políticas"]
        else:
            opcoes = ["Avulsos", "Políticas"]
        
        pagina_selecionada = st.sidebar.selectbox("Selecione a Página", opcoes, key="selectbox_pagina")
        
        if pagina_selecionada == "Configurações":
            pagina_dados()
        elif pagina_selecionada == "Avulsos":
            pagina_avulsos()
        elif pagina_selecionada == "Políticas":
